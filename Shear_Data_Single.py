...
def shear_quadrants(lon, lat, storm_lon, storm_lat, shear_dir):
    ...
    if 0<shear_dir<90:
        ...
        dr_quad = np.where((lon>(storm_lon+dx))&(lat>(storm_lat+dy)))[0]
        dl_quad = np.where((lon>(storm_lon+dx))&(lat<(storm_lat+dy)))[0]
        ur_quad = np.where((lon<(storm_lon+dx))&(lat>(storm_lat+dy)))[0]
        ul_quad = np.where((lon<(storm_lon+dx))&(lat<(storm_lat+dy)))[0]
