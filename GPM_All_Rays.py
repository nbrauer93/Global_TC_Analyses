...
for file in range(len(files)):
    ...
    if ku.shape[0] != 7933:
        ...
        updated_lat.append(lat_update)
        updated_lon.append(lon_update)
        ...
        #If no difference, then keep KuPR the same
    


array_kupr = np.stack(updated_list)
array_lat = np.stack(updated_lat)
array_lon = np.stack(updated_lon)
...
for i in range(len(xlim_west)):    
   ...
   ind1 = np.where((array_lon[i,:,0]>=xlim_west[i])) #Where are the DPR lons >= -100
   ind2 = np.where((array_lon[i,:,0])<=xlim_east[i]) #Where are lons <= -85
...
lat_3d = np.ones(lats.shape)*np.nan
lon_3d = np.ones(lons.shape)*np.nan
